...
class RandomFourierFeatures(tf.keras.layers.Layer):
	...
	def call(self, inputs):
		...
		self.add_loss(tf.abs((outputs-tf.expand_dims(tf.math.reduce_min(outputs, axis=-1), axis=-1))/tf.expand_dims(tf.math.reduce_max(outputs,axis=-1)-tf.math.reduce_min(outputs, axis=-1), axis=-1)-(outputs)/(2*np.pi)))
