...
class RandomFourierFeatures(tf.keras.layers.Layer):
	...
	def __init__(self, output_dim, kernel_initializer='gaussian', scale=None, trainable=False, units=None, regularizer=None, seed=None, name=None, **kwargs):
		...
		self.layer_normalization=tf.keras.layers.LayerNormalization(beta_initializer=tf.constant_initializer(np.pi/2), gamma_initializer=tf.constant_initializer(np.pi), trainable=False)
		self.scale = scale
		self.seed=seed
