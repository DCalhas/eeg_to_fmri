...
class variational_iDCT3D(tf.keras.layers.Layer):
	...
	def call(self, x):
		...
		if(self.dependent):
			rand_coefs1 = rand_coefs1*tf.matmul(a=tf.reshape(x, (x.shape[0], x.shape[1]*x.shape[2]*x.shape[3],)), b=self.w)
			rand_coefs2 = rand_coefs2*tf.matmul(a=tf.reshape(x, (x.shape[0], x.shape[1]*x.shape[2]*x.shape[3],)), b=self.w)
			rand_coefs3 = rand_coefs3*tf.matmul(a=tf.reshape(x, (x.shape[0], x.shape[1]*x.shape[2]*x.shape[3],)), b=self.w)
