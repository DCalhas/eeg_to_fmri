...
class variational_iDCT3D(tf.keras.layers.Layer):
	...
	def call(self, x):
		...
		if(self.distribution=="VonMises"):
			...
			rand_coefs1=tf.cos((1/self.scale)*cartesian_dist1.sample())
			rand_coefs2=tf.cos((1/self.scale)*cartesian_dist2.sample())
			rand_coefs3=tf.cos((1/self.scale)*cartesian_dist3.sample())
