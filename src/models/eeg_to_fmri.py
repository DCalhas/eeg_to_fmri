    ...
    def build_decoder(self, input_shape, output_encoder, latent_shape, fourier_features=False, random_fourier=False, 
        ...
        if(inverse_DFT):
            if(variational_iDFT):
                ...
                x = variational_iDCT3D(*(latent_shape + self.fmri_ae.in_shape[:3] + variational_coefs), coefs_perturb=True)(x)
            else:
...
class pretrained_EEG_to_fMRI(tf.keras.Model):
    ...
    def build_decoder(self, pretrained_model, input_shape, output_encoder, activation=None, attention_scores=None, regularizer=None, seed=None):
        ...
        sigma_1 = tf.keras.layers.Dense(1)(sigma_1)#, activation=tf.keras.activations.exponential)(sigma_1)
        sigma_2 = tf.keras.layers.Dense(1)(sigma_2)#, activation=tf.keras.activations.exponential)(sigma_2)
