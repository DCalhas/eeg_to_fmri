...
if __name__ == "__main__":
	...
	parser.add_argument('-save_weights_path', default=str(Path.home())+"/eeg_to_fmri/src/softmax_weights", type=str, help="Path of directory to save save weights.")
...
for epoch in range(epochs):
	...
	for batch in range(1,n_batches+1):
		...
		o_predictions = np.zeros((networks,batch_size,x_dim,y_dim,z_dim), dtype=np.float32)
		...
		for network in range(networks):
			...
			o_predictions[network]=np.array(flattened_predictions).reshape((batch_size,x_dim,y_dim,z_dim))
			

#removing batch directory
shutil.rmtree(batch_path)

#process_utils.launch_process(process_utils.load_data_latent_fmri, (dataset, n_individuals, n_individuals_train, n_volumes, interval_eeg, memory_limit))