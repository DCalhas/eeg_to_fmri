...
def entropy_mae_loss(y_true, y_pred):
    regression = (1/(y_pred[3]))*tf.reduce_mean(tf.abs(y_pred[1]-y_pred[2]), axis=(1,2,3))
    classification = (1/y_pred[4])*tf.reduce_mean(tf.math.square(tf.cast(y_true, y_pred[0].dtype)-y_pred[0]), axis=(1,2,3))#tf.keras.losses.CategoricalCrossentropy(from_logits=True)(y_true, y_pred[0]) 
    return classification + regression + tf.math.log(2*y_pred[4]*y_pred[3])
