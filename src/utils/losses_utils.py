...
class ContrastiveClassificationLoss(tf.keras.losses.Loss):
    ...
    def call(self, y_true, y_pred, y1_var=1., y2_var=1.):
        ...
        return 0.*tf.reduce_mean(self.contrastive(y_contrast, y_pred[1]), axis=1)+(1/y1_var)*self.nll(tf.expand_dims(y_clf1[:,1],-1), y1_pred)+0.*(1/y2_var)*self.nll(tf.expand_dims(y_clf2[:,1],-1), y2_pred) + tf.math.log(y1_var*y2_var)
