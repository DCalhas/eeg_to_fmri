...
def train_synthesis(dataset, epochs, style_prior, padded, variational, variational_coefs, variational_dependent_h, variational_dist, variational_random_padding, resolution_decoder, aleatoric_uncertainty, fourier_norm, batch_norm_reg, save_path, gpu_mem, seed, run_eagerly, verbose):
	...
	with tf.device('/CPU:0'):
		...
		model = eeg_to_fmri.EEG_to_fMRI(latent_dimension, eeg_train.shape[1:], na_specification_eeg, n_channels,
							...
							topographical_attention=True, conditional_attention_style=True, conditional_attention_style_prior=style_prior, batch_norm_reg=batch_norm_reg,
...
def loocv(fold, setting, view, dataset, l2_regularizer, epochs, optimizer_name, learning_rate, batch_size, gpu_mem, seed, run_eagerly, save_explainability, path_network, path_labels, feature_selection=False, segmentation_mask=False, aleatoric_uncertainty=False, style_prior=False, variational=False, verbose=False):
	...
	if(save_explainability and (view=="fmri" or view=="fmri_nll")):
		#explaing features
