...
def multi_modal_network(eeg_input_shape, bold_input_shape, eeg_network, bold_network, dcca=False, dcca_output=None, lstm=False, corr_distance=False, gan=False, gan_activation="softmax", gan_output_dimension=1):
    input_eeg = tf.keras.layers.Input(shape=eeg_input_shape)
    ...
    if(corr_distance):
        ...
        correlation = tf.keras.layers.Dense(gan_output_dimension, activation=gan_activation)(correlation)#softmax activation for a probability output does not work as it outputs zeroed gradients
    else:
...
def grad_decoder_adversarial(discriminator, synthesizer, z, eeg, clip=True, clip_value=2, loss=losses_utils.loss_minmax_generator):
    ...
    for epoch in range(epochs):
        ...
        for batch_init in range(0, len(X_train_eeg), batch_size):
            ...
            with tf.name_scope("gradient_decoder") as scope:
                decoder_grads, _ = tf.clip_by_global_norm(decoder_grads, clip_value_gradient)
    ...
    for epoch in range(epochs):
        ...
        for batch_init in range(0, len(X_train_eeg), batch_size):
            ...
            encoder_loss, encoder_grads = grad_multi_encoder(multi_modal_model, 
                                                             [tf.gather_nd(eeg_train, X_train_eeg[batch_start:batch_stop]), 
                                                                                 tf.gather_nd(bold_train, X_train_bold[batch_start:batch_stop])], 
                                                             tr_y[batch_start:batch_stop], 0, linear_combination, clip_value=clip_value_loss) #decoder loss 0
            with tf.name_scope("gradient_encoders") as scope:
                encoder_clipped_grads = ()
                for grad in encoder_grads:
                    encoder_clipped_grads += (tf.clip_by_value(grad, clip_value_min=-clip_value_gradient, clip_value_max=clip_value_gradient), )
